...
# Ensure each column has the same number of values
student_data <- student_data[1:35, ]
...
kmeans_model <- kmeans(scaled_data, centers = 3)  # Adjust number of centers as needed

# Add cluster assignments to the original dataset
student_data$Cluster <- as.factor(kmeans_model$cluster)

# Visualize the clusters
ggplot(student_data, aes(x = Marks, y = Age, color = Cluster)) +
  geom_point() +
  labs(title = "K-means Clustering of Students",
       x = "Marks",
       y = "Age",
       color = "Cluster") +
  theme_minimal()
